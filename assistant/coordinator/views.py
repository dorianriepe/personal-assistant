...
def reminder(request):
    if request.method == "POST":
        ...
        morning_default = datetime.strptime(
            request.POST['morning_reminder'], '%H:%M')
        shopping_default = datetime.strptime(
            request.POST['shopping_reminder'], '%H:%M')
        cooking_default = datetime.strptime(
            request.POST['cooking_reminder'], '%H:%M')
        evening_default = datetime.strptime(
            request.POST['evening_reminder'], '%H:%M')

        morning = now.replace(hour=morning_default.hour,
                              minute=morning_default.minute)
        shopping = now.replace(hour=shopping_default.hour,
                               minute=shopping_default.minute)
        cooking = now.replace(hour=cooking_default.hour,
                              minute=cooking_default.minute)
        evening = now.replace(hour=evening_default.hour,
                              minute=evening_default.minute)
        ...
        for event in todays_events:
            event_start = datetime.strptime(
                event['start'], '%H:%M') + timedelta(minutes=-10)
            event_start = now.replace(
                hour=event_start.hour, minute=event_start.minute)
            reminders.append({"ts": event_start.strftime(
                "%H:%M"), "hour": event_start.hour, "minute": event_start.minute, "usecase": "meeting"})

        if now < morning:
            reminders.append({"ts": morning.strftime(
                "%H:%M"), "hour": morning.hour, "minute": morning.minute, "usecase": "welcome"})

        if now < shopping:
            reminders.append({"ts": shopping.strftime(
                "%H:%M"), "hour": shopping.hour, "minute": shopping.minute, "usecase": "hungry"})

        if now < cooking:
            reminders.append({"ts": cooking.strftime(
                "%H:%M"), "hour": cooking.hour, "minute": cooking.minute, "usecase": "hungry"})

        if now < evening:
            reminders.append({"ts": evening.strftime(
                "%H:%M"), "hour": evening.hour, "minute": evening.minute, "usecase": "night"})
